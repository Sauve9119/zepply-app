...
// FIX: pehle yahan alag hardcoded fallback secret tha ('zepply_secret_2024') jo
// middleware/auth.js ke fallback se match nahi karta tha — agar JWT_SECRET env
// var set na ho toh login ke baad tokens invalid ho jaate the. Ab single source
// of truth (middleware/auth.js) se import karte hain.
const token = id => jwt.sign({ userId: id }, JWT_SECRET, { expiresIn: '7d' });
...
const sendOTP = async (email, otp, subject, body) => {
  ...
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: { name: 'Qdoor App', email: process.env.BREVO_SENDER_EMAIL },
        to: [{ email }],
        subject,
        htmlContent: `
          <div style="font-family:Arial,sans-serif;max-width:400px;margin:0 auto;padding:20px">
            <h2 style="color:#F97316">🛒 Qdoor</h2>
            <p>${body}</p>
            <div style="background:#f4f4f4;padding:20px;border-radius:8px;text-align:center;margin:20px 0">
              <h1 style="letter-spacing:8px;color:#F97316;margin:0">${otp}</h1>
            </div>
...
router.post('/send-otp', async (req, res) => {
  ...
  sendOTP(email, otp, 'Qdoor — Email Verify karo', 'Aapka Qdoor registration OTP:');
});
...
router.post('/register', async (req, res) => {
  try {
    ...
    const refCode = 'QDR' + Math.floor(1000 + Math.random() * 9000);
    const newUser = {
    ...
    db.insert('notifications', { id: 'n' + uid(), user_id: newUser.id, title: 'Qdoor mein aapka swagat hai! 🎉', body: `Namaste ${name}! Order karo, review do aur doston ko refer karo — ZepCoins kamao!`, read: false, created_at: new Date().toISOString() });
